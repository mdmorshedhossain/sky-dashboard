...
const Welcome = () => {
    ...
    //           setRefetch(!refetch)
    //           toast("Image inserted successfully");
    //       });
    // }
    ...
    const deleteImage = (id) =>{
        ...
        fetch(`http://localhost:5000/deleteWelcome/${id}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
            }
...
//         console.log(image,updatedId);

//         fetch(`http://localhost:5000/updateimage/${updatedId}`, {
//            method: "PUT",
//            headers: {
//              "Content-Type": "application/json",
//            },
    ...
    return (
        <div style={{margin:'30px 10px 0px 10px', width:'100%',heigh:'full'}} className=''>
                ...
                <div style={{zIndex:'12', background:'#f2eded'}} className='rounded flex flex-col justify-center items-center border absolute h-[300px] w-[700px] top-[120px] right-[230px] shadow-lg'>
                    <div className='flex flex-col'>
                        <form
                            ...
                            action='http://localhost:5000/welcome'
                            method='post'
                            encType='multipart/form-data'
                        >
                            <div>
                                <input name='title' className='w-full rounded-sm px-2 text-sm h-8' type="text" placeholder='Title' /> <br />
                                <textarea name='description' className='mt-2 w-full rounded-sm px-2 text-sm' id="" cols="30" rows="2" placeholder='Descriptions'></textarea> <br />
                                <input
                                className='mt-2'  
                                type="file" 
                                name="welcomeImage"/>
            ...
            {/*Edit image modal */}
            {
                ...
                <div style={{zIndex:'12', background:'#f2eded'}} className='rounded flex flex-col justify-center items-center border absolute h-[300px] w-[700px] top-[120px] right-[230px] shadow-lg'>
                    <div>
                        <h1 className='pb-4 text-2xl font-semibold'>Update welcome page</h1>
                        <form 
                            action={`http://localhost:5000/editWelcome`}
                            method='post'
                            encType='multipart/form-data'
                            >
                            <input name='title' className='w-full rounded-sm px-2 text-sm h-8' type="text" placeholder='Title' /> <br />
